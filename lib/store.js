...
// 조율 등급은 v2.3부터 **사람이 아니라 수업**에 붙는다(programs[].flex).
// 옛 파일의 teachers[].grade 는 읽지도 쓰지도 않는다 — 다만 지우지도 않는다.
// 쓰지 않게 된 값이라고 남의 자료를 말없이 걷어 내지는 않는다.
...
function ensureShape(d) {
  ...
  // 우리 학교만 쉬는 날(재량휴업일·방학). 옛 파일에는 없으므로 읽을 때 빈 배열로 채운다.
  if (!Array.isArray(d.room.schoolHolidays)) d.room.schoolHolidays = [];
  // 신청하려는 프로그램 목록(마스터만 본다). 옛 파일에는 이 칸이 없다 —
  // 마이그레이션으로 파일을 고치지 않고 읽을 때 빈 배열로 채운다.
  if (!Array.isArray(d.room.plans)) d.room.plans = [];
  d.room.plans = d.room.plans.map(function (p) {
    return {
      id: (p && p.id) || genId('pl'),
      title: String((p && p.title) || ''),
      teacherIds: Array.isArray(p && p.teacherIds) ? p.teacherIds.slice() : [],
      memo: String((p && p.memo) || ''),
      applied: !!(p && p.applied),
  ...
  d.room.teachers = d.room.teachers.map(function (t, i) {
    ...
    // 옛 파일에 남아 있는 grade 는 그대로 지고 간다(쓰지는 않는다)
    if (t && t.grade !== undefined) out.grade = t.grade;
    return out;
  });
  return d;
}
...
module.exports = {
  ...
  PALETTE: PALETTE, genId: genId, pruneCorrupt: pruneCorrupt,
